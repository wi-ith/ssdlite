...
boxes=[[0.1, 0.2, 0.5, 0.6],[0.2, 0.1, 0.3, 0.4]]
labels=[1., 2.]
...
anchor_size=feature.get_shape().as_list()[1:3]
ratio_list=np.float32(np.array([2.,1.,1./2.]))
scale=0.65
boxes=tf.convert_to_tensor([[0.2, 0.1, 0.3, 0.4],[0.1, 0.2, 0.5, 0.6]])
...
anchor_x1y1=tf.stack([row_coordi,col_coordi],axis=2)/anchor_size[0]
anchor_x2y2=anchor_x1y1+1/anchor_size[0]
...
max_iou_idx = tf.argmax(iou,axis=-1) +2
...
# positive label index(+2 from original input to make index for negative and ignorance)
pos_label_idx = max_iou_idx*(1-tf.cast(negative_mask,dtype=tf.float32))*(1-tf.cast(ignore_mask,dtype=tf.float32))
